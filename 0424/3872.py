...
if menu == "편평시험(Flattening) 계산":
    ...
    if d_val > 0:
        ...
        st.metric(label="최종 목표 높이 (H)", value=f"{h_val:.2f} mm")
        st.metric(label="압착 필요량 (Press)", value=f"{press_val:.2f} mm", delta=f"-{press_val:.2f} mm", delta_color="inverse")
        
        st.warning(f"💡 {item} 규격에 따라 외경의 {ratio_str}까지 압착합니다.")
        
        with st.expander("📝 계산 상세 보기"):
            st.write(f"- 파이프 외경: {d_val} mm")
            st.write(f"- 적용 기준: {ratio_str}")
            st.write(f"- 계산식: {d_val} × {ratio_str} = {h_val:.4f}")
            st.write(f"- 압착량 계산: {d_val} - {h_val:.2f} = {press_val:.2f}")
    else:
